...
export const renderBatteryElements = (
	...
	// Khung Pin (V/A/W) là "flow" -- đồng bộ với icon pin (đã dùng
	// flowBatColour từ trước): theo màu nguồn chiếm ưu thế (PV/Lưới) khi sạc
	// khi bật config.battery.dynamic_colour, else batteryColour tĩnh.
		...
		>
			...
			>
				${renderText(
					...
					'st3 left-align',
					data.stateDayBatteryCharge.toNum() > 0 ? batteryColour : 'grey',
				...
				${renderText(
					...
					data.batteryShowDaily !== true,
					'st3 left-align',
					data.stateDayBatteryDischarge.toNum() > 0 ? batteryColour : 'grey',
			...
			<svg id="battery-flow">
				${renderPath(
					...
					config.wide
						...
						: 'M 155 280 L 96 280 Q 86 280 86 290 L 86 300.75',
					true,
					// Đường Pin <-> Inverter: khi Inverter Standby thì chuyển
					// grey, tắt animation. LUÔN batteryColour riêng (bỏ mixing
					// động theo PV/Lưới sạc pin -- màu pha trộn giờ chỉ dùng
					// cho DOT animation).
					data.isInverterStandby ? 'grey' : batteryColour,
					data.batLineWidth,
				)}
				${renderCircle(
					'power-dot-discharge',
				...
				${renderCircle(
					...
					'#bat-line',
					config.battery.invert_flow,
					data.batteryChargeCycleColours,
				)}
			</svg>
		</svg>
	`;

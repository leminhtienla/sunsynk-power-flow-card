...
export const renderLoadElements = (
	...
	// Text "Daily Load" (nhãn + giá trị kWh): grey khi kWh = 0, loadColour
	// khi > 0.
	const loadDailyColour =
		data.stateDayLoadEnergy.toNum() > 0 ? loadColour : 'grey';

	// Màu Home Load chính (khung + icon nhà + label W + đường es-line/
	// es-line2): LUÔN dùng loadColour riêng của nhóm (giống "Daily Load"
	...
	return html`
		...
		>
			...
			<rect
				x="${[1, 2].includes(config.solar?.mppts) ? '261' : '276'}"
				y="103"
				width="70"
				height="30"
				rx="4.5"
			...
			<svg id="load1-flow">
				...
				${renderCircle(
					...
					data.displayEssentialPower <= 0
						...
						: homeLoadColour,
					data.durationCur['load'],
					// Path 'es-line' đầu = Grid-tie (y=153), cuối = Load (y=133).
					// Công suất dương: Grid → Load, tức đi từ đầu (0) đến cuối (1) path.
					'0;1',
					'#es-line',
					config.load.invert_flow === true,
					data.homeLoadCycleColours,

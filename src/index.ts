...
import { pathLengthCache } from './helpers/path-length-cache';
import {
	CARD_VERSION,
	EDITOR_NAME,
	MAIN_NAME,
...
export class SunsynkPowerFlowCard extends LitElement {
	...
	private _pathLengthMeasurePending = false;
	...
	// Lưu màu nguồn EPS (PV/Pin) hiển thị lần render gần nhất, dùng làm vùng
	// đệm chống nháy màu khi 2 nguồn xê xích quanh mốc 50/50 (xem
	// epsSourceColour bên dưới).
	private _epsSourceColourPrev: string | null = null;
	...
	// Sau mỗi lần render, đo lại độ dài thực tế (px) của các đường SVG có
	// animation, dùng để điều chỉnh tốc độ (dur) tỉ lệ theo độ dài — đường
	// dài/ngắn khác nhau nhưng cùng % công suất sẽ giữ đúng tốc độ px/giây.
	updated(changedProps: PropertyValues): void {
		super.updated(changedProps);
		if (this._pathLengthMeasurePending) return;
		this._pathLengthMeasurePending = true;
		requestAnimationFrame(() => {
			this._pathLengthMeasurePending = false;
			this._measurePathLengths();
		});
	}

	private _measurePathLengths(): void {
		...
		paths.forEach((el) => {
			...
			// Bỏ qua đường có độ dài 0 (chưa layout xong hoặc display:none) để
			// tránh cache giá trị sai tạm thời.
			if (!Number.isFinite(length) || length <= 0) return;
			const prev = pathLengthCache.get(id);
			// Chỉ coi là "thay đổi thật" nếu lệch > 0.5px, tránh vòng lặp
			// requestUpdate vô ích do sai số làm tròn cực nhỏ.
			if (prev === undefined || Math.abs(prev - length) > 0.5) {
				pathLengthCache.set(id, length);
				changed = true;
			}
		});
	...
	render() {
		...
		//EPS / backup voltage & frequency
		const stateEpsVoltage = this.getEntity('entities.eps_voltage');
		const stateEpsFrequency = this.getEntity('entities.eps_frequency');
		//Grid connected status (on-grid vs EPS/off-grid)
		const stateGridConnected = this.getEntity('entities.grid_connected');
		const stateInverterStatusText = this.getEntity(
			'entities.inverter_status_text',
		);
		...
		const gridVoltage = !stateGridVoltage.isNaN()
			...
			: 0;
		const gridFrequency = !stateGridFrequency.isNaN()
			? stateGridFrequency.toNum(1)
			: 0;
		...
		// Màu TĨNH gốc (config.load.aux_colour), KHÔNG phụ thuộc công suất EPS
		// tức thời như auxDynamicColour -- dùng cho "Daily EPS" (label + kWh)
		// để không bị grey nhầm khi công suất EPS hiện tại = 0W nhưng tổng kWh
		// tích lũy trong ngày vẫn > 0 (giống bug đã sửa cho Daily Solar).
		const auxBaseColour = this.colourConvert(config.load?.aux_colour);
		...
		const inverterVoltage = config.entities?.inverter_voltage_154
			...
			: 0;
		const epsVoltage = config.entities?.eps_voltage
			? stateEpsVoltage.toNum(1)
			: 0;
		const epsFrequency = config.entities?.eps_frequency
			? stateEpsFrequency.toNum(2)
			: 0;
		// Ưu tiên entity grid_connected nếu có khai báo. Nếu chưa khai báo,
		// tự suy luận dự phòng: coi như mất lưới khi điện áp Grid-tie
		// (inverter_voltage_154) gần 0V (< 30V), vì đây là dấu hiệu rõ ràng
		// nhất khi hệ thống đang chạy độc lập chế độ EPS/off-grid.
		const gridConnected = config.entities?.grid_connected
			? stateGridConnected.state === 'on'
			: !(config.entities?.inverter_voltage_154 && inverterVoltage < 30);
		const inverterStatusText = config.entities?.inverter_status_text
			? stateInverterStatusText.state
			: '';
		...
		// Màu PV TĨNH (không bị dim về grey theo công suất PV hiện tại như
		// solarColour) -- dùng cho label cố định "DAILY SOLAR" và cho giá trị
		// kWh khi >0, để không bị grey nhầm khi PV hiện tại=0W nhưng tổng kWh
		// trong ngày vẫn >0 (VD buổi tối, PV đã ngừng phát nhưng đã tích lũy).
		const solarBaseColour = this.colourConvert(config.solar?.colour);
		...
		// Ô "Inverter": khi bám lưới hiện công suất grid-tie (inverter_power_175),
		// khi mất lưới (chạy EPS độc lập) hiện công suất EPS thực tế (auxPower = eps_power).
		const displayInverterPower = gridConnected
			? autoScaledInverterPower
			: auxPower;

		// Ô "Home Load": khi mất lưới, I_PLOAD không còn đúng thiết kế (chỉ hợp lệ on-grid),
		// nên ép về 0 để tránh hiện số nhiễu/dao động quanh giá trị EPS Power.
		const displayEssentialPower = gridConnected ? essentialPower : 0;
		...
		let found = false;
		// Đánh dấu riêng nhóm status "standby" theo GROUP KEY thực (không
		// phải chuỗi message đã localize, để không bị sai khi đổi ngôn ngữ).
		let inverterStatusCodeIsStandby = false;
		...
		if (typeStatusGroups)
			for (const groupKey of Object.keys(typeStatusGroups)) {
				...
				if (states.includes(inverterState.toLowerCase())) {
					...
					inverterStatusCodeIsStandby = groupKey === 'standby';
					found = true;
					break;
				}
			}
		...
		// Standby nếu status CODE thuộc nhóm 'standby' (theo statusGroups của
		// từng hãng inverter) HOẶC status TEXT (entity riêng, chuỗi tự do)
		// đúng bằng "standby" (không phân biệt hoa/thường). Dùng để chuyển
		// grey + tắt animation các đường liên quan đến inverter.
		const inverterStatusTextIsStandby =
			config.entities?.inverter_status_text &&
			stateInverterStatusText.state?.trim().toLowerCase() === 'standby';
		const isInverterStandby =
			inverterStatusCodeIsStandby || !!inverterStatusTextIsStandby;
		...
		// Mốc công suất tối đa DÙNG CHUNG cho toàn bộ animation (PV, Pin,
		// Load, Grid, Inverter) — đại diện công suất thực tế của Inverter
		// (VD Inverter 6kW thì để max_power ~9kW). Thay vì mỗi loại dùng 1
		// mốc riêng (gây tốc độ animation lệch nhau dù % công suất tương
		// đương), giờ tất cả quy về CÙNG 1 mốc, ưu tiên grid.max_power (gần
		// đúng công suất Inverter nhất), rồi lần lượt các mốc khác nếu chưa
		// cấu hình grid.max_power.
		const unifiedMaxPower =
			gridMaxPower.toNum(0) ||
			loadMaxPower.toNum(0) ||
			batteryMaxPower.toNum(0) ||
			solarMaxPower.toNum(0) ||
			0;
		...
		// Đường es-line/es-line2 (Grid-tie→Load→icon) dùng displayEssentialPower
		// cho animation (tự ép về 0 khi mất lưới), nên độ dày cũng cần khớp
		// theo giá trị này, không dùng loadLineWidth (dựa trên essentialPower
		// gốc, không bị ép về 0).
		const displayEssentialLineWidth = !config.load.max_power
			? minLineWidth
			: this.dynamicLineWidth(
					Math.abs(displayEssentialPower),
					loadMaxPower.toNum() || Math.abs(displayEssentialPower),
					maxLineWidth,
					minLineWidth,
				);
		...
		// Độ dày đường Inverter↔Grid-tie và đoạn dây gốc Inverter: khớp đúng
		// biến autoScaledInverterPower đang điều khiển animation của 2 đường
		// này (grid2-line, inverter-path), thay vì để cố định/dùng nhầm biến
		// grid power khác không liên quan.
		const inverterLineWidth = !config.grid.max_power
			? minLineWidth
			: this.dynamicLineWidth(
					Math.abs(autoScaledInverterPower),
					gridMaxPower.toNum() || Math.abs(autoScaledInverterPower),
					maxLineWidth,
					minLineWidth,
				);
		// Đoạn dây gốc Inverter (inverter-path) dùng displayInverterPower
		// (tự đổi giữa grid-tie/EPS tùy trạng thái lưới), nên cần biến độ
		// dày riêng khớp đúng giá trị này, không dùng chung inverterLineWidth.
		const inverterStubLineWidth = !config.grid.max_power
			? minLineWidth
			: this.dynamicLineWidth(
					Math.abs(displayInverterPower),
					gridMaxPower.toNum() || Math.abs(displayInverterPower),
					maxLineWidth,
					minLineWidth,
				);
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				...
				(config.solar.animation_speed - 1) *
					(totalPV / (unifiedMaxPower || totalPV));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) *
					(pv1PowerWatts / (unifiedMaxPower || pv1PowerWatts));
			this.changeAnimationSpeed(`pv1`, speed);
		}

		// Đường vẽ "pv2" hiển thị dữ liệu PV3 đã hoán (xem pv_elements.ts),
		// nên tốc độ animation cũng phải tính theo pv3PowerWatts, không phải
		// pv2PowerWatts gốc — và ngược lại cho "pv3".
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) *
					(pv3PowerWatts / (unifiedMaxPower || pv3PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) *
					(pv2PowerWatts / (unifiedMaxPower || pv2PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) *
					(pv4PowerWatts / (unifiedMaxPower || pv4PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) *
					(pv5PowerWatts / (unifiedMaxPower || pv5PowerWatts));
		...
		if (config && config.solar && config.solar.animation_speed) {
			const speed =
				config.solar.animation_speed -
				(config.solar.animation_speed - 1) *
					(pv6PowerWatts / (unifiedMaxPower || pv6PowerWatts));
		...
		if (config && config.battery && config.battery.animation_speed) {
			const speed =
				...
				(config.battery.animation_speed - 1) *
					(Math.abs(batteryPowerTotal) /
						(unifiedMaxPower || Math.abs(batteryPowerTotal)));
		...
		if (config && config.load && config.load.animation_speed) {
			const speed =
				...
				(config.load.animation_speed - 1) *
					(Math.abs(essentialPower) /
						(unifiedMaxPower || Math.abs(essentialPower)));
		...
		if (config && config.load && config.load.animation_speed) {
			const speed =
				config.load.animation_speed -
				(config.load.animation_speed - 1) *
					(Math.abs(auxPower) / (unifiedMaxPower || Math.abs(auxPower)));
		...
		if (config && config.grid && config.grid.animation_speed) {
			const speed =
				...
				(config.grid.animation_speed - 1) *
					(Math.abs(totalGridPower) /
						(unifiedMaxPower || Math.abs(totalGridPower)));
			this.changeAnimationSpeed(`grid1`, speed);
			this.changeAnimationSpeed(`grid`, speed);
		}

		// Đường "grid2-line" (Inverter↔Grid-tie) và "inverter-path" (đoạn dây
		// gốc Inverter) không phản ánh công suất mua/bán lưới thực tế
		// (totalGridPower), mà phản ánh công suất Inverter xuất ra
		// (autoScaledInverterPower / displayInverterPower) — nên cần tốc độ
		// animation tính riêng theo đúng biến này, không dùng chung với
		// nhóm `grid`/`grid1` ở trên.
		if (config && config.grid && config.grid.animation_speed) {
			const speed =
				config.grid.animation_speed -
				(config.grid.animation_speed - 1) *
					(Math.abs(autoScaledInverterPower) /
						(unifiedMaxPower || Math.abs(autoScaledInverterPower)));
		...
		if (config && config.grid && config.grid.animation_speed) {
			const speed =
				...
				(config.grid.animation_speed - 1) *
					(Math.abs(displayInverterPower) /
						(unifiedMaxPower || Math.abs(displayInverterPower)));
			this.changeAnimationSpeed(`inverter`, speed);
		}
		...
		if (config && config.grid && config.grid.animation_speed) {
			const speed =
				...
				(config.grid.animation_speed - 1) *
					(Math.abs(nonessentialPower) /
						(unifiedMaxPower || Math.abs(nonessentialPower)));
		...
		// % PV / Pin cấp cho EPS (dựa theo auxPower, CHỈ 2 nguồn vì EPS cách ly
		// hoàn toàn khỏi Lưới -- không có nhánh Grid như Home Load). Dùng
		// chung mẫu tính như pvPercentageRaw/batteryPercentageRaw ở trên,
		// nhưng thay essentialPower bằng auxPower làm mẫu số.
		const pvPercentageRawEps =
			auxPower > 0 ? Utils.toNum((totalPV / auxPower) * 100, 0) : 0;
		const batteryPercentageRawEps =
			config.battery.invert_flow === true
				? batteryPowerTotal >= 0
					? 0
					: auxPower > 0
						? Utils.toNum((Math.abs(batteryPowerTotal) / auxPower) * 100, 0)
						: 0
				: batteryPowerTotal <= 0
					? 0
					: auxPower > 0
		...
		// Chuẩn hóa để PV%+Pin% luôn = 100% khi auxPower>0 (chỉ 2 nguồn, không
		// còn phần dư như Home Load có Lưới).
		const totalPercentageEps = pvPercentageRawEps + batteryPercentageRawEps;
		const pvPercentageEps =
			totalPercentageEps > 0
				? Utils.toNum((pvPercentageRawEps / totalPercentageEps) * 100, 0)
				: 0;
		const batteryPercentageEps =
			totalPercentageEps > 0
		...
		let epsIcon: string;
		switch (true) {
			case pvPercentageEps >= 100 &&
				batteryPercentageEps <= 5 &&
				config.load.dynamic_icon:
				epsIcon = icons.essPv;
				break;
			case batteryPercentageEps >= 100 &&
				pvPercentageEps <= 5 &&
				config.load.dynamic_icon:
				epsIcon = icons.essBat;
				break;
			default:
				epsIcon = icons.aux;
				break;
		...
		// Màu nguồn chiếm ưu thế cấp cho EPS (chỉ PV hoặc Pin, theo ngưỡng
		// path_threshold của battery -- dùng chung ngưỡng với battery vì EPS
		// đi qua cùng đường dẫn nội bộ Inverter->Battery).
		// Luôn chọn bên PV hoặc Pin đang chiếm ưu thế hơn (so sánh trực tiếp %,
		// KHÔNG dùng path_threshold) -- vì EPS chỉ có 2 nguồn duy nhất, một
		// trong hai luôn nên thắng thế thay vì rơi về inverterColour mặc định
		// khi không ai đạt ngưỡng (VD path_threshold mặc định 100% khiến
		// hầu như không bao giờ đạt trong thực tế pha trộn PV+Pin).
		// Có vùng đệm 10%: nếu chênh lệch PV%-Pin% < 10 điểm %, GIỮ NGUYÊN
		// màu đang hiển thị lần render trước -- tránh nháy màu qua lại khi 2
		// nguồn xê xích quanh mốc 50/50.
		const epsPercentDiff = Math.abs(pvPercentageEps - batteryPercentageEps);
		...
		// Mảng màu DOT cho đường sạc Pin (thứ tự PV -> Lưới, chỉ 2 nguồn có
		// thể sạc pin, không tính chính Pin). Dùng cho dot "power-dot-charge"
		...
		// Màu dùng chung cho "nhóm EPS" (khung V/Hz EPS, label "EPS", label
		// "EPS LOAD", icon nhà EPS, đường aux-line): grey khi công suất EPS
		// hiện tại (auxPower) = 0, else theo epsSourceColour -- CHỈ phân biệt
		// PV/Pin (không có Lưới, vì EPS cách ly hoàn toàn khỏi Lưới) khi bật
		// cả 2 toggle dynamic_colour, else inverterColour tĩnh. Tính tập
		// trung ở đây để mọi nơi dùng chung 1 nguồn duy nhất, tránh lệch
		// nhóm giữa icon/text/khung/đường dây.
		// ĐƠN GIẢN HÓA (sửa lớn theo yêu cầu): khung/text/đường KHÔNG còn pha
		// trộn màu động (PV/Pin) nữa -- chỉ dùng auxBaseColour RIÊNG của nhóm
		...
		const data: DataDto = {
			...
			gridConnected,
			inverterStatusText,
			displayInverterPower,
			displayEssentialPower,
			batteryVoltage,
			battery2Voltage,
			stateBatteryCurrent,
			...
			solarLineWidth,
			inverterLineWidth,
			inverterStubLineWidth,
			displayEssentialLineWidth,
			totalPV,
			loadLineWidth,
			pvPercentageBat,
			gridPercentageBat,
